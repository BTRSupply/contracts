...
library LibSwapper {
    ...
    function getRestrictions() internal view returns (uint256) {
        return S.restrictions().swapperMask;
    }

    function setRestriction(uint8 _bit, bool _value) internal {
        uint256 restrictions = S.restrictions().swapperMask;
        S.restrictions().swapperMask = _value 
    ...
    function executeSwap(
        ...
        // Validate access
        if (isCallerRestricted(_caller) ||
            isRouterRestricted(_router) ||
    ...
    function initialize(
        ...
        S.restrictions().swapperMask = restrictions;
